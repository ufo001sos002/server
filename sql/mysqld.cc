...
/**
  ...
  @param arg  optional parameter to leaves out those threads whose server command
              is as of specified by the parameter.
  @returns 0 as success, otherwise 1.
*/
static my_bool kill_all_threads(THD *thd, void *arg)
{
  ...
  if (thd->slave_thread)
    return 0;
  if (arg &&
      (enum_server_command) * (uint32_t*) arg == thd->get_command())
    return 0;
...
/**
  ...
  @returns 0 as success, otherwise 1.
*/
static my_bool kill_all_threads_once_again(THD *thd, void *arg)
{
#ifndef __bsdi__				// Bug in BSDI kernel
  /* slow shutdown leaves out dump threads */
  if (arg &&
      (enum_server_command) * (uint32_t*) arg == thd->get_command())
    return 0;
...
static void close_connections(void)
{
  ...
  */
  ...
  uint32_t binlog_dump_desc= (uint32_t) COM_BINLOG_DUMP;
  my_bool is_slaves_wait_shutdown=
    mysql_bin_log.slaves_wait_shutdown == MYSQL_BIN_LOG::SHDN_PREPARE;
  server_threads.iterate(kill_all_threads,
                         !is_slaves_wait_shutdown ? NULL :
                         (void*) &binlog_dump_desc);
